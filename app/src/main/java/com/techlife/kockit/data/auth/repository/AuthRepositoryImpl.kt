...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun registerWithGoogle(
        ...
        return when (val result = authRemoteDataSource.registerWithGoogle(registerInfo, oAuthIdToken)) {
            is ApiResult.Success -> {
                authLocalDataSource.persistRegistration(registerInfo, result.data)
                result
            }
    ...
    override suspend fun logout() {
        // Önce sunucuya çıkış bildir (Authorization header için token henüz duruyor),
        // ardından lokal access/refresh token ve oturum her koşulda silinir.
